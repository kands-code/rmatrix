...
/// # }
/// ```
pub fn read<T>(path: &std::path::Path, index: usize) -> IResult<Matrix<T>>
where
    T: Clone + for<'a> serde::Deserialize<'a>,
{
    let full_path = match path.canonicalize() {
        Ok(full_path) => Ok(full_path),
        Err(_) => Err(IError::Message(format!("path {:?} is wrong", path))),
    }?;
    let mut file = match std::fs::File::open(&full_path) {
        Ok(handle) => Ok(handle),
        Err(_) => Err(IError::Message(format!(
            "file {:?} is not exist",
            full_path
        ))),
    }?;
...
/// # }
/// ```
pub fn write<T>(mats: &[&Matrix<T>], path: &std::path::Path) -> IResult<()>
where
