...
impl<N, const R: usize, const C: usize> Matrix<N, R, C> {
    ...
    pub(crate) fn position_to_index(row_index: usize, column_index: usize) -> usize { (row_index - 1) * C + column_index - 1 }
    ...
    ///
    /// ```rust
    /// use rmatrix_ks::{matrix::Matrix, number::instances::word8::Word8};
    ///
    ...
    /// use rmatrix_ks::{matrix::Matrix, number::instances::word8::Word8};
    ///
    /// let m = Matrix::<Word8, 2, 2>::fills(Word8::of(2));
    /// let m_expect = Matrix::<Word8, 2, 2>::of(&[2, 2, 2, 2].map(Word8::of)).unwrap();
    /// assert_eq!(m, m_expect);
    /// ```
    pub fn fills(data: N) -> Self
    where
        N: Clone,
    {
        Self {
    ...
    ///
    /// ```rust
    /// use rmatrix_ks::{matrix::Matrix, number::instances::double::Double};
    ///
                    ...
                    .flat_map(|e| {
                        (0..C)
                            .map(|p| {
                                e.clone().power(
                                    N::from_str(&format!("{p:?}"))
                                        .expect("Error[Matrix::vandermonde]: Failed to convert from usize."),
                                )
                            })
                            .collect::<Vec<N>>()
                    })
    ...
    /// let n =
    ///     Matrix::<Word8, 3, 3>::of(&[7, 8, 9, 4, 5, 6, 1, 2, 3].map(|e| Word8::of(e))).unwrap();
    /// assert_eq!(p * m, n);
    /// ```
    ...
    ///
    /// ```rust
    /// use rmatrix_ks::{matrix::Matrix, number::instances::int8::Int8};
    ///
    /// let m =
    ///     Matrix::<Int8, 3, 3>::of(&[1, 2, 3, 4, 5, 6, 7, 8, 9].map(|e| Int8::of(e))).unwrap();
    /// let p = Matrix::<Int8, 3, 3>::p_add(1, 2, Int8::of(-4));
    /// let n =
    ///     Matrix::<Int8, 3, 3>::of(&[1, 2, 3, 0, -3, -6, 7, 8, 9].map(|e| Int8::of(e))).unwrap();
    /// assert_eq!(p * m, n);
    /// ```
    ...
    ///
    /// ```rust
    /// use rmatrix_ks::{matrix::Matrix, number::instances::float::Float};
    ///
    ...
    pub fn get(&self, row_index: usize, column_index: usize) -> Option<&N> {
        if row_index == 0 || column_index == 0 || row_index > R || column_index > C {
            eprintln!("Error[Matrix::get]: Index ({row_index}, {column_index}) is out of bounds.");
            None
        } else {
    ...
    pub fn set(&mut self, row_index: usize, column_index: usize, value: N) {
        if row_index == 0 || column_index == 0 || row_index > R || column_index > C {
            eprintln!("Error[Matrix::set]: Index ({row_index}, {column_index}) is out of bounds.");
        } else {
            let position = Self::position_to_index(row_index, column_index);
    ...
    pub fn get_row(&self, row_index: usize) -> Option<VectorR<&N, C>> {
        if row_index == 0 || row_index > R {
            eprintln!("Error[Matrix::get_row]: Index ({row_index}) is out of bounds.");
            None
        } else {
    ...
    pub fn get_column(&self, column_index: usize) -> Option<VectorC<&N, R>> {
        if column_index == 0 || column_index > C {
            eprintln!("Error[Matrix::get_column]: Index ({column_index}) is out of bounds.");
            None
        } else {
...
impl<N, const R1: usize, const C1: usize, const R2: usize, const C2: usize> std::cmp::PartialEq<Matrix<N, R2, C2>>
    for Matrix<N, R1, C1>
where
...
impl<N, const R: usize, const K: usize, const C: usize> std::ops::Mul<Matrix<N, K, C>> for Matrix<N, R, K>
where
    N: Number,
{
