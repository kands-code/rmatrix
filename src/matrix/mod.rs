...
impl<N> Matrix<N> {
    ...
    /// let n =
    ///     Matrix::<Word8>::of(3, 3, &[7, 8, 9, 4, 5, 6, 1, 2, 3].map(|e| Word8::of(e))).unwrap();
    /// assert_eq!(p * m, n);
    /// ```
    ...
    ///
    /// let m =
    ///     Matrix::<Int8>::of(3, 3, &[1, 2, 3, 4, 5, 6, 7, 8, 9].map(|e| Int8::of(e))).unwrap();
    /// let p = Matrix::<Int8>::p_add(3, 3, 1, 2, Int8::of(-4));
    /// let n =
    ///     Matrix::<Int8>::of(3, 3, &[1, 2, 3, 0, -3, -6, 7, 8, 9].map(|e| Int8::of(e))).unwrap();
    /// assert_eq!(p * m, n);
    /// ```
    ...
    pub(crate) fn position_to_index(column: usize, row_index: usize, column_index: usize) -> usize {
    ...
    pub fn get(&self, row_index: usize, column_index: usize) -> Option<&N> {
        if row_index == 0 || column_index == 0 || row_index > self.row || column_index > self.column {
